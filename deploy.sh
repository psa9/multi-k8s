...
docker build -t psa39/multi-server-k8s-pgfix:latest -t psa39/multi-server-k8s-pgfix:$SHA -f ./server/Dockerfile ./server
...
docker push psa39/multi-server-k8s-pgfix:latest
docker push psa39/multi-worker-k8s:latest

docker push psa39/multi-client-k8s:$SHA
docker push psa39/multi-server-k8s-pgfix:$SHA
docker push psa39/multi-worker-k8s:$SHA

kubectl apply -f k8s
kubectl set image deployments/server-deployment server=psa39/multi-server-k8s-pgfix:$SHA
